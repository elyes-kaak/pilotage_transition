...
class Cinetique:
    ...
    def cinetique_demande(self, t, t_crise):
        ...
        return gama * self.cinetique_demande(t_crise, 1000)
    ...
    def surcout_trajectoire(self, x, t_crise):
        a1 = integrate.quad(lambda t : self.evol_taxe(t, x, c1i, beta_c1) * self.cinetique_demande(t, t_crise), 0, x[2])[0] # taxe dans la phase carbonée
        a2 = integrate.quad(lambda t : self.evol_taxe(t, x, c1i, beta_c1) * self.cinetique_techno(t, t_crise, x), x[2], t_f)[0] # taxe sur x1 de t1 à tf
        a3 = integrate.quad(lambda t : self.evol_taxe(t, x, c2i, beta_c2) * (self.cinetique_demande(t, t_crise) - self.cinetique_techno(t, t_crise, x)), x[2], t_f)[0]  # taxe sur x2 de 0 à tf
    ...
    def eq_evol(self, x, t_crise):
        ...
        taxe_1, taxe_2 = [], []
        for t in temps :
            taxe_1.append(self.evol_taxe(t, x, c1i, beta_c1))
            taxe_2.append(self.evol_taxe(t, x, c2i, beta_c2))
