...
class Methodes :
    def methode_budget(self) :
        ...
        plot = Plot(temps, evol_couts, evol_techno, type_techno, demande, evol_taxes)
        print('Emissions de la solution (crise) : ', cinetique.budget_carbone(x, crise))
        print('Surcout (crise) : ', cinetique.surcout_trajectoire(x, crise))
        plot.plot()
    ...
    def methode_surcout(self) :
        ...
        optimisation = Optim(lambda x : cinetique.surcout_trajectoire(x, 1000), contraintes.contr, bnds, x0, lambda x : cinetique.budget_carbone(x, 1000), lambda x : cinetique.surcout_trajectoire(x, 1000))
